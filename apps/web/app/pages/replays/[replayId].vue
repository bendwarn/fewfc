...
function cardTokensForRefs(cards: PublicCardRefs | undefined, prefix: string): CardToken[] {
  if (!cards) return []
  if (cards.kind === 'known') {
    return cards.cards.map((card, index) => ({ id: `${prefix}-known-${index}-${card.id}`, label: card.label, element: card.element, level: card.level }))
  }
  if (cards.kind === 'partiallyKnown') {
    return cards.cards.map((card, index) => card
      ? { id: `${prefix}-known-${index}-${card.id}`, label: card.label, element: card.element, level: card.level }
      : { id: `${prefix}-hidden-${index}`, label: '', element: null, level: null })
  }
  return Array.from({ length: cards.count }, (_, index) => ({ id: `${prefix}-hidden-${index}`, label: '', element: null, level: null }))
}
...
.lobby-content { @apply grid gap-5; }.lobby-heading { @apply flex items-center justify-between gap-4; }.lobby-heading h1 { @apply font-serif text-3xl text-gold-light; }.result-actions { @apply mt-2 grid grid-cols-2 gap-3; }.result-actions .ghost-button { @apply border-[#59635c] text-[#e8e4d8]; }
.replay-layout { @apply block; }.replay-battlefield { @apply min-h-[620px]; }
.battlefield { position: relative; display: grid; grid-template-areas: 'top top top' 'left center right' 'bottom bottom bottom'; grid-template-columns: minmax(108px,.6fr) minmax(300px,1.8fr) minmax(108px,.6fr); grid-template-rows: minmax(150px,.8fr) minmax(230px,1.15fr) minmax(175px,1fr); gap: 8px 14px; padding: 22px 36px; overflow: hidden; background: radial-gradient(ellipse at center,#273029 0%,#141b17 58%,#0f1512 100%); }
...
.hand { @apply flex min-w-0 items-center justify-center gap-2; }.playing-card { width: clamp(62px,7vw,92px); aspect-ratio: 5 / 7; border: 1px solid var(--card-border,#79715e); border-radius: 5px; --card-face:#ded8c8; --card-face-light:#f0eadc; --card-border:#79715e; --card-ink:#18201c; background-image: linear-gradient(145deg,var(--card-face-light),var(--card-face)); color:var(--card-ink); @apply relative flex flex-col items-center justify-center p-2; }.card-level { @apply absolute top-1 left-1/2 -translate-x-1/2 font-serif text-xs; }.card-element { @apply grid size-[35px] place-items-center rounded-full border border-current font-serif text-lg; }.playing-card.element-Metal { --card-face:#ddd5b5;--card-face-light:#f3eed8;--card-border:#89783e;--card-ink:#67571e; }.playing-card.element-Wood { --card-face:#cfe0ce;--card-face-light:#e8f1e5;--card-border:#52765a;--card-ink:#315f3d; }.playing-card.element-Water { --card-face:#cbdfe8;--card-face-light:#e7f1f5;--card-border:#4d7890;--card-ink:#245d78; }.playing-card.element-Fire { --card-face:#ead0c9;--card-face-light:#f6e7e2;--card-border:#985448;--card-ink:#8d3026; }.playing-card.element-Earth { --card-face:#e4d5ba;--card-face-light:#f3ead8;--card-border:#936d3b;--card-ink:#785027; }
.board-center { grid-area:center; @apply relative z-1 grid min-w-0 grid-cols-[90px_minmax(220px,1fr)_90px] items-center justify-items-center; }.discard-piles { @apply z-2 flex w-full justify-end; grid-column:1/-1;grid-row:1; }.discard-pile { @apply grid justify-items-center gap-1.5 text-[9px] text-[#707b73]; }.formation-field { @apply relative z-3 grid min-h-48 w-full min-w-0 grid-rows-[auto_1fr_auto] items-center border-x border-[rgba(166,141,86,.14)] px-3 py-2 text-center text-[10px] text-[#69736c]; grid-column:2;grid-row:1; }.formation-field-heading { @apply flex flex-wrap items-center justify-center gap-2; }.formation-field-label { @apply text-[#9a8251];letter-spacing:.2em; }.environment-badge { @apply border border-[#64583f] bg-[#1a211c] px-2 py-1 text-[9px] text-gold-light; }.previous-formation { @apply grid min-h-24 content-center justify-items-center gap-1.5; }.previous-formation small { @apply text-[9px] text-muted; }.previous-formation strong { @apply font-serif text-sm text-gold-light; }.previous-formation p { @apply text-[10px] text-[#68726b]; }.action-detail { @apply absolute right-0 bottom-[calc(100%+8px)] left-0 z-8 border border-[#64583f] bg-[#1c241f] p-3 text-left text-xs leading-5 text-muted shadow-[0_12px_28px_rgba(0,0,0,.4)]; }.event-panel { @apply min-h-0 overflow-auto border-b border-line bg-panel p-5; }.panel-title { @apply flex items-start justify-between; }.panel-title h2 { @apply font-serif text-[15px]; }.event-feed { @apply mt-4 grid list-none gap-[13px] p-0; }.event-feed li { @apply grid grid-cols-[10px_1fr] gap-[7px]; }.event-feed span { @apply text-[10px] font-bold text-[#d4d8d4]; }.event-feed p { @apply mt-0.5 text-[9px] leading-5 text-[#6f7972]; }

...
End-to-end workflow for authoring and maintaining Playwright tests with `bun playwright cli`. Every `bun playwright cli` action emits the equivalent Playwright TypeScript, and that generated code is the raw material for every test. The sections below can be used independently:
...
Plan / generate / heal lean on the same mechanic: run `bun playwright test --debug=cli` in the background, then `bun playwright cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics.

---

## 0. How generation works

Every action you perform with `bun playwright cli` generates corresponding Playwright TypeScript code. This code appears in the output and can be copied directly into your test files.

```bash
# Start a session
bun playwright cli open https://example.com/login
...
test('login flow', async ({ page }) => {
  // Generated code from bun playwright cli session:
...
Use `bun playwright cli generate-locator <target>` to produce the locator expression for the assertion, and the snapshot/eval commands to capture the expected value.
...
bun playwright cli --raw generate-locator e5
# getByRole('button', { name: 'Submit' })

# Capture expected text content for toHaveText
bun playwright cli --raw eval "el => el.textContent" e5

# Capture expected input value for toHaveValue/toBeEmpty
bun playwright cli --raw eval "el => el.value" e5

# Capture expected aria snapshot for toMatchAriaSnapshot/toBeChecked
# (whole page, or use a ref to scope to a region)
bun playwright cli --raw snapshot
bun playwright cli --raw snapshot e5
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bun playwright test tests/seed.spec.ts --debug=cli
# wait for "Debugging Instructions" and the session name tw-XXXX
bun playwright cli attach tw-XXXX
```

Resume so the seed runs, then probe the app:

```bash
bun playwright cli resume                   # resume so that seed test runs fully
bun playwright cli snapshot                 # inventory of interactive elements
bun playwright cli click e5                 # follow a flow
bun playwright cli eval "location.href"     # read URL / state
bun playwright cli show --annotate          # ask the user to point at something
```
...
**Important**: Do not just open the app url with bun playwright cli, always go through the test to capture any custom setup done there.
...
```bash
PLAYWRIGHT_HTML_OPEN=never bun playwright test <seed-file> --debug=cli   # background
bun playwright cli attach tw-XXXX
# resume
```

**Do not** just open the app url with bun playwright cli, always go through the test to capture any custom setup done there.

Walk the scenario's `Steps:` one by one with `bun playwright cli`, treating the spec as the plan and the live app as the source of truth. If a step is vague ("click the button" — which button?), references an element that no longer exists, or contradicts the app's actual behaviour, use your judgement: update the spec to match what the app really does, then keep going. Editing the spec mid-generation is expected.

Every action prints the equivalent Playwright TypeScript (see [How generation works](#0-how-generation-works)):

```bash
bun playwright cli snapshot                         # find refs
bun playwright cli fill e3 "John Doe"               # -> page.getByRole('textbox', {...}).fill(...)
bun playwright cli press Enter
bun playwright cli click e7
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bun playwright test tests/<group>/<scenario>.spec.ts
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bun playwright test
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bun playwright test tests/<group>/<scenario>.spec.ts:<line> --debug=cli
...
```bash
bun playwright cli snapshot                # did the element change / move / rename?
bun playwright cli console                 # app-side errors?
bun playwright cli requests                # failed request? wrong payload?
bun playwright cli show --annotate         # ask the user to point somewhere
```

Common causes: selector drift, new wrapper element, label/ARIA rename, timing (transition, async load), assertion text updated in the app, test data leaking between runs.

Rehearse the corrected interaction with `bun playwright cli` — the generated code in the output is what you paste back into the test.

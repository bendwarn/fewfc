...
#[test]
fn falling_wood_pays_a_printed_element_cost_and_echoes_next_turn_start() {
    let mut state = configured_state();
    state.hp[0].hp = 170;
    ...
    assert!(events.iter().any(|event| matches!(
        event,
        GameEvent::HpChanged { change } if change.new_hp == 185
    )));
    assert!(
        events
            .iter()
    ...
    assert_eq!(state.scheduled_echoes[0].due_turn_number, 3);
    assert!(state.discard.contains(&card(21)));

    state.turn_number = 3;
    state.current_turn_index = 0;
    state.phase = Phase::TurnStart;
    let echoed = advance_automatic(&state).unwrap();
    assert!(matches!(
        echoed.first(),
        Some(GameEvent::EchoResolutionStarted { .. })
    ));
    assert!(echoed.iter().any(|event| matches!(
        event,
        GameEvent::HpChanged { change } if change.old_hp == 185 && change.new_hp == 200
    )));
    assert!(
        echoed
            .iter()

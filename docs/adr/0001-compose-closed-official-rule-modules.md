...
Advanced Rule Modules from a closed official catalog. A Ruleset configuration
is the Base Ruleset plus its enabled modules; Game Setup and Game State retain
that configuration so command handling, automatic advancement, Formation
queries, replay, and Public View use the same rule knowledge without caller
selection.
...
- A public trait for downstream rule modules was rejected because one known
  optional module does not justify freezing an external interface.
...
Adding an official Advanced Rule Module requires extending the exhaustive
catalog and may extend the shared canonical schema. The Star Rule Module is
implemented end to end, including a default-on Web room toggle and Public View
data; disabling it leaves the complete Base Ruleset active. Before the first
public release, persisted Game Record compatibility across schema changes is not
guaranteed; no version or migration mechanism is added yet.
